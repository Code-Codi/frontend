...
const Schedule = () => {
  ...
  // 임시 데이터
  const scheduleData = [
    { startDate: LocalDateTime.parse('2025-05-12T17:00:00'), endDate: LocalDateTime.parse('2025-05-13T18:00:00'), title: '겹치는 일정 4' },
    { startDate: LocalDateTime.parse('2025-05-13T18:00:00'), endDate: LocalDateTime.parse('2025-05-21T19:00:00'), title: '겹치는 일정 3' },
    { startDate: LocalDateTime.parse('2025-05-15T18:00:00'), endDate: LocalDateTime.parse('2025-05-21T19:00:00'), title: '겹치는 일정 5' },
    { startDate: LocalDateTime.parse('2025-05-15T18:00:00'), endDate: LocalDateTime.parse('2025-05-21T19:00:00'), title: '겹치는 일정 5' },
    { startDate: LocalDateTime.parse('2025-05-15T18:00:00'), endDate: LocalDateTime.parse('2025-05-21T19:00:00'), title: '겹치는 일정 5' },
    { startDate: LocalDateTime.parse('2025-05-15T18:00:00'), endDate: LocalDateTime.parse('2025-05-21T19:00:00'), title: '겹치는 일정 5' },
  ...
  const navigateToPreviousMonth = () => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() - 1));
  const navigateToNextMonth = () => setCurrentDate(new Date(currentDate.getFullYear(), currentDate.getMonth() + 1));
  ...
  const createScheduleWeeks = () => {
    ...
    const daysArray = Array(firstDay).fill(null).concat(Array.from({ length: lastDay }, (_, i) => i + 1));
  ...
  const getScheduleItemsForDate = (schedules) => {
    ...
    schedules.forEach((schedule) => {
      ...
      placedSchedules.forEach(placed => {
        if (!(endDate.isBefore(placed.startDate) || startDate.isAfter(placed.endDate))) {
  ...
  const isWithinRange = (dateObj, item) => {
    ...
    const start = new Date(startDate.year(), startDate.monthValue() - 1, startDate.dayOfMonth());
    const end = new Date(endDate.year(), endDate.monthValue() - 1, endDate.dayOfMonth());
    ...
    <div>
      <ScheduleContainer>
        <ScheduleHeader>
          ...
          <MonthText>{`${currentYear}.${String(currentDate.getMonth() + 1).padStart(2, '0')}`}</MonthText>
                ...
                return (
                  ...
                  >
                    ...
                    {day !== null && (() => {
                      ...
                      const { scheduleItems } = getScheduleItemsForDate(scheduleData);
                      const filteredItems = scheduleItems.filter(item => isWithinRange(dateObj, item));
                      return (
                        <>
                          {filteredItems.slice(0, 3).map((item, idx) => {
                            const showTitle = item.startDate.equals(
                              LocalDate.of(dateObj.getFullYear(), dateObj.getMonth() + 1, dateObj.getDate())
                            );

                            const isEndOrSaturday = LocalDate.of(dateObj.getFullYear(), dateObj.getMonth() + 1, dateObj.getDate())
                              .equals(item.endDate) || dateObj.getDay() === 6;

                            return (
                              <ScheduleItem 
                                key={idx} 
                                width={`${isEndOrSaturday ? cellWidth - 2 : cellWidth}px`} 
                                line={item.line}
                              >
